	...
	for (int i = 0; i < iterations ; i++) {
		...
		if (r > 2) 
		{
			float smooth_col =  float(i) +
				...
				//col = texture2D(iGradient, vec2(smooth_col / 10.5, 1)).rgb * 0.3 / (smooth_col / 12.5);
				col = vec3(0.3) / (smooth_col / 12.5);
			break;
		}
...
float AmbientOcclusion(vec3 point, vec3 normal, float step_dist, float step_nbr)
{
    ...
    while(step_nbr > 0.0)
    {
	occlusion -= pow(step_nbr * step_dist - (mbulb_de( point + normal * step_nbr * step_dist, 7, 8)),2) / step_nbr;
	step_nbr--;
    }
...
bool march_ray(Ray r, Mandelbulb m, out float fraction, out vec3 normal, out vec3 col)
{
	...
	for(int i = 0; i < 150; i++)
	{
		...
		if(d < 0.0002)
		{
			...
			col = max(1.0 - (i / 120.0), 0.0) * color;
			//col = color * pow(AmbientOcclusion(pos, normal, 0.0001, 20), 40);
			fraction = d;
			return true;
		}
		else if(d > 10)
...
void scene_init()
{
	...
	boxes[0].halfSize = vec3(500.0, 500.0, 1.0);
	boxes[0].rotation = rot(0, 0, 0);
	
	mbulbs[0].material = blue;
	mbulbs[0].radius = 1;
	mbulbs[0].power = 8;
	mbulbs[0].iterations = 8;
	mbulbs[0].rotation = rot(2 * 3.14159, 0.75, 1.68);
	mbulbs[0].position = vec3(0, 0, 0.8);
}
